/**
 * Static JEE Syllabus data — mirrors backend/syllabus_data.py
 * Used directly by the frontend to avoid dependency on a backend endpoint.
 */
...
export function getStructuredSyllabus() {
  const result = {};
  for (const [subjectSlug, groups] of Object.entries(SUBJECT_GROUPS)) {
    const subjectName = slugToTitle(subjectSlug);
    const subjectData = [];
    for (const groupSlug of groups) {
      const chaptersInGroup = GROUP_CHAPTERS[groupSlug] || [];
      const chapterList = [];
      for (const chSlug of chaptersInGroup) {
        const topics = (CHAPTER_TOPICS[chSlug] || []).filter(Boolean);
        chapterList.push({
          chapter: slugToTitle(chSlug),
          chapter_slug: chSlug,
          topics: topics.map(t => ({ name: slugToTitle(t), slug: t })),
        });
      }
      subjectData.push({
        group: slugToTitle(groupSlug),
...
export function getTotalTopicCount() {
  let total = 0;
  const syl = getStructuredSyllabus();
  for (const groups of Object.values(syl)) {
